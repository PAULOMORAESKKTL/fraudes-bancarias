...
def analisar_transacoes(transacoes):
    ...
    """
    try:
        print("1 - Iniciou a analise de FRAUDE...")
        ...
        print("2 - Finalizou a analise de FRAUDE")
...
def gerar_parecer(transacao):
    ...
    try:
        ...
        resposta = mensagem.content[0].text
        print("4 - finalizou a geração do parecer!")
        return resposta
    except anthropic.APIConnectionError as e:
        print("O servidor não pode ser acessado! Erro:", e.__cause__)
    except anthropic.RateLimitError as e:
...
def gerar_recomendacao(parecer):
    ...
    try:
        ...
        resposta = mensagem.content[0].text
        print("6 - finalizou a geração recomendação!")
        return resposta
    except anthropic.APIConnectionError as e:
        print("O servidor não pode ser acessado! Erro:", e.__cause__)
    except anthropic.RateLimitError as e:
...
transacoes = carrega('dados/transacoes.csv')
...
for transacao in transacoes_analisadas["transacoes"]:
    if transacao["status"] == "Possível Fraude":
        ...
        salva(f'transacao-{transacao["id"]}-{transacao["nome_produto"]}-{transacao["status"]}.txt',
              recomendacao)